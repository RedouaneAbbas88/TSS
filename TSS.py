...
def load_sheet_df(sheet_name):
    try:
        ...
        df.columns = df.columns.str.strip()  # enlever espaces invisibles
        return df
    ...
    if st.sidebar.button("Se connecter"):
        if df_users.empty:
            ...
            if user_row.empty:
                ...
                # On ne hash pas ici, comparer directement
                if user_row['Password'] != password_input:
                    ...
                    st.success(f"Connecté : {st.session_state.user_name} — {st.session_state.user_role}")
...
if st.session_state.logged_in:
    st.title(f"Interface après connexion — {st.session_state.user_name} ({st.session_state.user_role})")

    # -----------------------------
    # Helper: stock distributeur en temps réel
    # -----------------------------
        ...
        with adv_tab[0]:
            ...
            with st.form("form_stock"):
                ...
                if submitted:
                    ...
                    st.experimental_rerun()  # pour actualiser l'état du stock
    ...
    # -----------------------------
        ...
        pre_tab = st.tabs(["Plan de visite", "Saisie commande", "Historique commandes"])
        ...
        with pre_tab[1]:
            ...
            if pos_options:
                ...
                st.write("Aucun POS à commander aujourd'hui.")

        # Historique commandes
        with pre_tab[2]:
            st.subheader("Historique des commandes")
            df_cmd = load_sheet_df(SHEET_COMMANDES)
            df_user_cmd = df_cmd[df_cmd['Code_Vendeur']==st.session_state.user_code_vendeur] if not df_cmd.empty else pd.DataFrame()
