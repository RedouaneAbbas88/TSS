...
if not st.session_state.logged_in:
    ...
    st.sidebar.success(f"Connecté : {user_email} — {user_role}")
                ...
                if not df_pending.empty:
                    for idx, row in df_pending.iterrows():
                        ...
                        if st.button(f"Valider commande {row['ID']}", key=row['ID']):
                            ...
                            update_cell(SHEET_COMMANDES, idx + 2, 'Valide_par', user_email)
    ...
    # -----------------------------
        ...
        with pre_tab_idx[1]:
            ...
            df_pos_today = df_list_pos[df_list_pos['Date_Visite'] == today_str]
